...
- **Browser-observable surface:** `no` — aSPARK Core is a Claude Code plugin made
  ...
  `62b4318`, the commit that removed the last script: 107 tracked files — 90
  `.md`, 5 `.yml` (GitHub issue templates and their `config.yml` — there is **no**
  CI workflow in this repo), 4 asset `.png`, 5 `.json` (2 plugin manifests + 3
  ...
  <!-- Pin history: originally `db4ab15` (106/4 `.json`, 2 reports). That commit
  was reworded to `62b4318` at /peer-review round 1 fix-mode (F7: a `feat!:`
  breaking-change marker contradicted its own body) via a non-interactive
  cherry-pick replay, which changes a commit's SHA — round 2 caught that the
  pin here had not followed it (F11, Major) and that a third report had landed
  on `origin/main` in the interim, so the count grew by one `.json` as well as
  by one commit. Re-pinned here at round 2 fix-mode. If this commit is reworded
  or rebased again, re-verify this pin before closing the round that does it. -->
...
- **Primary user:** `inferred from README.md:1-9` — a Claude Code user who wants a gated agile delivery loop, not an unstructured copilot.
- **Problem today:** `inferred from README.md:9-11` — nothing stops moving forward with an unclarified spec, an unreviewed diff or an unverified feature.
- **Smallest version:** `not found` — the product brief was drafted from evidence with no interview, and no file states an MVP slice for this meta-project; the product *is* the whole installed plugin, not a slice within it (`inferred from README.md:136-142`).
- **Success signal:** `inferred from ROADMAP.md:14-24` — a full loop run on someone else's real project, lens layer verified. This project's biggest open gap.
- **Stack:** `inferred from .claude-plugin/plugin.json:1-10` — Markdown + two JSON manifests; no runtime, build or test suite of its own.
- **Non-negotiables:** `inferred from .spark/constitution.md:188-191,196-200` — no silent breaking change to the consumed contract; nothing executed unasked; no agent passes its own gate.
...
- **How to run:** `inferred from README.md:172-175` — local: `claude --plugin-dir /path/to/aSPARK`; published: `/plugin install aspark@aspark`.
- **Known pain points:** `inferred from ROADMAP.md:14-24,47-49` — never proven on someone else's real project; lens compliance is instruction-driven, unenforced; gates are prompt-enforced, not code-enforced (issue #13).
