...
- **Open:** `4 outstanding` — three rulings only the user can give (AC-1.3's gap, QA `B1`, QA `B2`) plus the publish go itself; see §3 and the KEEP GATE.
...
| `B2` | QA Minor/informational: the ignored `docs/…Handbook.docx.bak` is confirmed present in the **local** install cache. Spec A9 already scopes this as a dev-surface leak; consumer reach via a real GitHub marketplace install is unverified and probably nil, and spec §6 routes it to a follow-up | Accept as informational and out of scope here |
