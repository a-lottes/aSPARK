...
- **Open:** `none` — A1–A5 and C1–C10 resolved; A6/A9 resolve as verified-downstream, named as such. **Note on C8–C10's authority:** all three are `/peer-review` findings (F3, F14, F16/F17) corrected by `/increment` fix-mode against the factual record — none required inventing a requirement or changing scope, and all are the kind of "small, obvious correction" fix-mode is authorised to make directly. None carries an explicit fresh user re-approval of the gate the way the constitution's A8 does. Round 3's Reviewer judged this disclosure honest but insufficient for C8 specifically — NFR-3's only measurable clause was reversed in the easier direction by the one remedy round 1 had excluded — and recorded that ratification is owed before release, not merely noted in passing. **Open for the user, explicitly: ratify C8 (and C9/C10) as recorded, or reopen the spec gate for a fresh walk.**
...
- [ ] AC-2.1: Given only `docs/reports/*.json` and the method as printed in `docs/metrics.md`, when a QA tester performs it (with `python3` or an untracked scratch script per §3/§5), then the observed output equals the figures currently published in `README.md`'s dated Snapshot block — checked against that block rather than a number fixed here, since a third report landing mid-feature already falsified one hardcoded set (F16) and a fourth would do it again.
...
- [ ] AC-3.2: Given `docs/reports/README.md`, when a reader looks for how to contribute a report, then the file states the directory is a closed, dated snapshot and that this repo contains no tool to produce another — verified by the absence in AC-3.1 plus the snapshot statement standing in the paragraph that once held the workflow.
...
| C10 | 2026-09-11 | `/peer-review` round 3 (F16, Major; F17, Minor): the same rebase C9 addressed also left AC-2.1 asserting the **pre-rebase** figures (10/54/54/53/52/41/48) with no escape clause, and AC-3.2 still naming `2026-09-09`. C9 generalised two ACs and missed two more carrying the identical defect. | **AC-2.1 points at the published Snapshot block instead of a fixed number** (same fix shape as C9, one step further: not just date-agnostic but figure-agnostic, since the figures are exactly what a fourth machine report would move next); **AC-3.2 generalised** the same way as AC-2.3. Neither spec edit carries a fresh explicit user re-approval — flagged in the Handoff alongside C8/C9, not claimed as settled |
...
- [x] Clarify pass done: no ambiguity left unresolved or unparked — C1–C7 all resolved at the spec gate; C8–C10 added post-approval from `/peer-review` findings, each resolved the same round it was raised
...
- [x] Line budget respected: Ist 190 / Soll ~250 (excluding HTML comments; +3 from C8–C10, all post-approval `/peer-review` corrections)
