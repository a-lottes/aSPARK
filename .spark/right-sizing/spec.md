...
**Handoff**
- **Status:** mirrors the header table above (authoritative for `Status`). **Approved by the user at the spec gate, 2026-08-29**, together with A7's confirmation. Next ceremony step: `/sprint-plan`.
...
- **Open:** `none` — A7 **confirmed by the user at the gate, 2026-08-29** (AC-1.10 stands: `/charter` only).
...
- [ ] AC-1.7: Given the shipped diff, when it is read, then the declaration is **specific to the browser/QA case**: it provides no generic mechanism to declare any other ceremony inapplicable, and `/story-time`, `/look-and-feel`, `/sprint-plan`, `/increment`, `/peer-review` and `/go-live` behave exactly as today at every value of it (C11).
...
| NFR-2 | Public surface (library lens §1) | Zero new slash commands, agents, lenses or template *files*. The whole added surface, enumerated line by line in the release notes: one optional constitution declaration (QA method only), and the reading/writing rules in `/demo-day`, `/go-live`, `/charter`, `qa-tester.md` and `reviewer.md`. A generic "declare any ceremony away" field, or any new template file, makes this a different feature (AC-1.7). | US-1, US-2 | `/peer-review` |
...
| NFR-4 | Degrade to silence (constitution §6) | Absent constitution, absent declaration, incomplete declaration, old-shape artifact: every ceremony behaves exactly as today — no error, no warning, no mention. The negative case is run first and written down (constitution §1). | US-1, US-2 | documented dogfood/dry run in a repo without the field |
...
- [x] Clarify pass done: no ambiguity left unresolved or unparked — second pass run on the revised two-story spec (C13–C16); C1–C12 resolved, C6/C7 explicitly superseded
...
- [x] Line budget respected: Ist 249 / Soll ~250 (excluding HTML comments — 4 comment lines, in §4 and §8)
- [x] Status set to `approved` by the user — 2026-08-29, in the `/story-time` gate walk, together with A7's confirmation
