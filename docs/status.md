...
| Ticket import + status write-back (`/story-time <issue>`, constitution §7 `Tracker write-back`) | **Shipped for GitHub Issues only; read and write proven separately, neither on an external project.** *Read:* a real `/story-time 19` fetched this repo's own public issue and the drafted spec cites it, seeded from its title and body; a nonexistent issue stopped cleanly; the no-argument path is word-for-word unchanged. *Write:* proven in a disposable private scratch repo (created, used, deleted) with write-back declared: a first `/go-live` posted exactly one comment (GitHub count 0 → 1, one `gh issue comment` call in the transcripts), and a second full release pass with the `Ticket comment` row recorded skipped it (count stayed 1). **Not proven live:** the `gh`-failure skip path and the `Ticket: none` skip (diff-level only), and the once-only guard's key is a hand-editable `release.md` row, so deleting it re-enables a post. This repo's own constitution declares `Ticket-reference format: none`, so the write half cannot run on it. No tracker other than GitHub Issues. Full ledger: [`.spark/ticket-import/evidence.md`](../.spark/ticket-import/evidence.md) |
